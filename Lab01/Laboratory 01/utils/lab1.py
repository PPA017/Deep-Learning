...
def abc2wav(abc_file):
    path_to_tool = os.path.join('./utils/bin', 'abc2wav')
    value = os.system('chmod +x "./utils/bin/abc2wav"')
    if value != 0:
        print(f"chmod error: {value}")
        return value
...
def test_custom_dense_layer_output(y):
    ...
    assert y.shape == true_y.shape, "[FAIL] output is of incorrect shape. expected {} but got {}".format(true_y.shape, y.shape)
